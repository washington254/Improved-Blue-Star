...
export default class Page {
    ...
    update() {
        ...
        //update simulation
        this.FBO.update();
// Adjust the frequency to slow down the oscillation
const frequency = 0.0003; // You can tweak this value to adjust the speed of oscillation

// Increment the amplitude value using a sinusoidal function
this.simulationShader.uniforms.amplitude.value = 40 + 10 * Math.sin(Date.now() * frequency);

// Ensure amplitude stays within the range of 20 and 40
if (this.simulationShader.uniforms.amplitude.value > 40.0) {
    this.simulationShader.uniforms.amplitude.value = 40.0;
} else if (this.simulationShader.uniforms.amplitude.value < 20.0) {
    this.simulationShader.uniforms.amplitude.value = 20.0;
}

        this.simulationShader.uniforms.timer.value += 0.01;
        this.FBO.particles.rotation.x = Math.cos( Date.now() *.001 ) * Math.PI / 180 * 2;
        this.FBO.particles.rotation.y -= Math.PI / 180 * .15;
    }
}
